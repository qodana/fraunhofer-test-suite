...
public class TestPassingPolymorphicValueToFunction {
  ...
  private static void foo2(HttpServletResponse response, Base var, String taint) throws IOException {
    response.getWriter().println(var.foo(taint)); //Inspection: Uncontrolled user input is passed to Cross-Site Scripting (XSS) sink
  }
}